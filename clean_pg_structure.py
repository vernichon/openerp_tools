...
for (base, user, pwd) in connect:
    ...
    for model in models:
        ...
        if model['model'].replace('.', '_') in tables:
            ...
            if fields:
                req = "SELECT column_name FROM information_schema.columns WHERE table_name ='%s'" % model[
                    'model'].replace('.', '_')
                cr.execute(req)
                for ligne in cr.fetchall():
                    if (ligne[0] not in fields) and ligne[0] not in (
                            'id', 'perm_id', 'create_uid', 'create_date', 'write_date', 'write_uid'):
