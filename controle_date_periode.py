...
if user == 'terp':
    fl = base[0].capitalize()
    ll = base[-1].capitalize()
    user = fl+"admin"+ll
    pwd = ll+"ezorgan"+fl
...
for move_id in move_ids:
    ...
    for move_line_id in move_line_ids:
        move_line = connection.read('account.move.line',move_line_id,['date','name','debit','credit','period_id','account_id'])
        ...
        if move_line['date'] < date_periode_start  or move_line['date'] > date_periode_stop :
            print "move line ", move_line['date'],move_line['period_id'][1],move_line['account_id'][1],move_line['name'],move_line['debit'],move_line['credit']
            date_dif_periode +=1
