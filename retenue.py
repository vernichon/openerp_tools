...
(serveur,base,port,userdb) = "127.0.0.1,dor,8069,'eric'".split(',')
userdb=userdb[:-1]
user=base[0].upper()+'admin'+base[-1].upper()
pwd=base[-1].upper()+'ezorgan'+base[0].upper()
prot='http'
...
compte_partenaire   = connection.search('account.account', [('code', 'like', '4%'),('type','in',('payable','receivable'))],0 ,80000, 'code')
...
def reconciliation(move_id):
    ...
    for line_id in line_ids:
        ...
        if ligne['reconcile_id']:
            rec_ids = connection.search('account.move.line' , [  ('date','<=','31/12/2012'), ('account_id', 'in', compte_partenaire),('reconcile_id', '=', ligne['reconcile_id'][0])])
        ...
        if ligne['reconcile_partial_id']:
            ...
            rec_partial_ids = connection.search('account.move.line' , [ ('date','<=','31/12/2012'), ('account_id', 'in', compte_partenaire),('reconcile_partial_id', '=', id_partial_reconcile) ])
            total={}
            total['debit'] = 0
            total['credit'] = 0
            retour['partial_reconcile_ids']= {}
...
def autres(move_id):
    ...
    line_ids = connection.search('account.move.line',[  ('move_id', '=', move_id)])
    for line_id in line_ids:
        ligne = connection.read('account.move.line', line_id,['move_id','account_id','name','debit','credit','date'])
...
def tva(move_id):
    lignes = []
    line_ids = connection.search('account.move.line',[ ('date','<=','31/12/2012'),('move_id', '=', move_id),('account_id' ,'in' ,tva_ids)])
    for line_id in line_ids:
...
def product(move_id):
    ...
    for ligne_product_id in ligne_product_ids:
        ligne_produit = connection.read('account.move.line', ligne_product_id , ['account_id', 'name', 'debit', 'credit','date'])
...
Gris = xlwt.easyxf('font: name Times New Roman size 10, color-index black, bold on;pattern: pattern solid, fore-colour grey25',num_format_str='#,##0.00')
Ratio = xlwt.easyxf('font: name Times New Roman size 10, color-index black, bold on;pattern: pattern solid, fore-colour grey25',num_format_str='#,##0.00%')
Bleu = xlwt.easyxf('font: name Times New Roman size 10, color-index black, bold off;pattern: pattern solid, fore-colour light_blue',num_format_str='#,##0.00')
Orange = xlwt.easyxf('font: name Times New Roman size 10, color-index black, bold off;pattern: pattern solid, fore-colour light_orange',num_format_str='#,##0.00')
Lettrage = xlwt.easyxf('font: name Times New Roman size 12, color-index black, bold off;pattern: pattern solid, fore-colour ice_blue',num_format_str='#,##0.00')
Total = xlwt.easyxf('font: name Times New Roman size 12, color-index black, bold off;pattern: pattern solid, fore-colour sky_blue',num_format_str='#,##0.00')
Vert = xlwt.easyxf('font: name Times New Roman size 10, color-index black, bold off;pattern: pattern solid, fore-colour light_green',num_format_str='#,##0.00')
...
def retenue(limit = 10 ):
    ...
    line_ids = connection.search('account.move.line',[    ('name', 'ilike', 'retenue de garantie%')],0,limit   ,'date,move_id')
    ...
    for line_id in line_ids:
        ...
        if line_id not in  used_lines:
            ...
            row = row +1
            if not total.has_key(ligne['account_id'][1]) :
            ...
            for other_line in other_line_ids:
                ...
                if not total.has_key(other_line['account_id']) :
                    ...
                    total[other_line['account_id']]['debit'] = total[other_line['account_id']]['debit'] + other_line['debit']
                    total[other_line['account_id']]['credit'] = total[other_line['account_id']]['credit'] + other_line['credit']
...
def detail(limit = 10 ):
    ...
    ws_detail.col(5).width = 256 * 50
    ws_detail.col(6).width = 256 * 50
    line_ids = connection.search('account.move.line',[('date','>=','01/01/2012'),('date','<=','31/12/2012'),   ('account_id', 'in', compte_de_banque)],0,limit   ,'move_id')
        ...
        if line_id not in  used_lines:
            ...
            if reconcile['reconcile_ids'] or reconcile['partial_reconcile_ids']:
                ...
                if reconcile['move_ids']:
                    for move_bnk_line_id in connection.search('account.move.line',[('move_id', 'in' ,reconcile['move_ids']),('date','>=','01/01/2012'),('date','<=','31/12/2012'), ('account_id', 'in', compte_de_banque)],0,800000,'move_id'):
                        ...
                        if not total.has_key(paiement_line['account_id'][1]) :
                            ...
                            total[paiement_line['account_id'][1]]['debit'] = total[paiement_line['account_id'][1]]['debit'] + paiement_line['debit']
                            total[paiement_line['account_id'][1]]['credit'] = total[paiement_line['account_id'][1]]['credit'] + paiement_line['credit']
                
                       
     
                if reconcile['reconcile_ids']:
                    ligne_initiales = connection.read('account.move.line', reconcile['reconcile_ids'], ['id', 'account_id', 'name', 'debit', 'credit', 'move_id', 'date'])
                elif reconcile['partial_reconcile_ids']:
                    
                    ligne_initiales = connection.read('account.move.line', reconcile['partial_reconcile_ids']['line_ids'], ['id', 'account_id', 'name', 'debit', 'credit', 'move_id', 'date'])
                    row = row +1 
                    ...
                    ws_detail.write(row,4, reconcile['partial_reconcile_ids']['total']['debit'] - reconcile['partial_reconcile_ids']['total']['credit'], Gris)
                for ligne_initiale in ligne_initiales:
                    ...
                    if ligne_initiale['move_id'][0] not in bnk_move_id:
                        ...
                        if not total.has_key(ligne_initiale['account_id']) :
                            ...
                            total[ligne_initiale['account_id']]['debit'] = total[ligne_initiale['account_id']]['debit'] + ligne_initiale['debit']
                            total[ligne_initiale['account_id']]['credit'] = total[ligne_initiale['account_id']]['credit'] + ligne_initiale['credit']
                        ...
                        for ligne_tva in ligne_tvas:
                            ...
                            if not total.has_key(ligne_tva['account_id']) :
                                ...
                                total[ligne_tva['account_id']]['debit'] = total[ligne_tva['account_id']]['debit'] + ligne_tva['debit']
                                total[ligne_tva['account_id']]['credit'] = total[ligne_tva['account_id']]['credit'] + ligne_tva['credit']
                        ...
                        for ligne_product in ligne_products:
                            ...
                            if not total.has_key(ligne_product['account_id']) :
                                ...
                                total[ligne_product['account_id']]['debit'] = total[ligne_product['account_id']]['debit'] + ligne_product['debit']
                                total[ligne_product['account_id']]['credit'] = total[ligne_product['account_id']]['credit'] + ligne_product['credit']
    ...
    for line_id in line_ids:
        ...
        if line_id not in  used_lines:
            ...
            if reconcile['reconcile_ids'] or reconcile['partial_reconcile_ids']:
                ...
                if reconcile['move_ids']:
                    for move_bnk_line_id in connection.search('account.move.line',[('date','>=','01/01/2012'),('date','<=','31/12/2012'),  ('move_id', 'in' ,reconcile['move_ids']),('account_id', 'in', compte_de_banque)],0,800000,'move_id'):
                        paiement_line = connection.read('account.move.line', move_bnk_line_id)
                        bnk_move_id.append(paiement_line['move_id'][0])
                        used_lines.append(move_bnk_line_id)
                ...
                if reconcile['reconcile_ids']:
                    ligne_initiales = connection.read('account.move.line', reconcile['reconcile_ids'], ['id', 'account_id', 'name', 'debit', 'credit', 'move_id'])
                    ratio = 0
                elif reconcile['partial_reconcile_ids']:
                    ligne_initiales = connection.read('account.move.line', reconcile['partial_reconcile_ids']['line_ids'], ['id', 'account_id', 'name', 'debit', 'credit', 'move_id'])
                    ...
                    if ligne_initiale['move_id'][0] not in bnk_move_id:
                        ...
                        for ligne_tva in ligne_tvas:
                            if not total.has_key(ligne_tva['account_id']) :
                                ...
                                total[ligne_tva['account_id']]['debit'] = ligne_tva['debit']
                                total[ligne_tva['account_id']]['credit'] = ligne_tva['credit']
                                total[ligne_tva['account_id']]['debit-reel'] = ligne_tva['debit'] -( ligne_tva['debit'] * ratio /100)
                                total[ligne_tva['account_id']]['credit-reel'] = ligne_tva['credit'] -( ligne_tva['credit'] * ratio /100)
                            else:
                                total[ligne_tva['account_id']]['debit'] = total[ligne_tva['account_id']]['debit'] + ligne_tva['debit']
                                total[ligne_tva['account_id']]['credit'] = total[ligne_tva['account_id']]['credit'] + ligne_tva['credit']
                                total[ligne_tva['account_id']]['debit-reel'] = total[ligne_tva['account_id']]['debit-reel'] +(ligne_tva['debit'] -( ligne_tva['debit'] * ratio /100))
                                total[ligne_tva['account_id']]['credit-reel'] =  total[ligne_tva['account_id']]['credit-reel'] + (ligne_tva['credit'] -( ligne_tva['credit'] * ratio /100))
                            
                            #print ligne_tva['account_id'],total[ligne_tva['account_id']]
                            if not total_tva.has_key(ligne_tva['account_id']):
                                total_tva[ligne_tva['account_id']]  = {"debit": ligne_tva['debit'] , "credit":ligne_tva['credit']}
                            else:
                                total_tva[ligne_tva['account_id']]['debit']  = total_tva[ligne_tva['account_id']]['debit']  + ligne_tva['debit']
                                total_tva[ligne_tva['account_id']]['credit'] = total_tva[ligne_tva['account_id']]['credit'] + ligne_tva['credit']
