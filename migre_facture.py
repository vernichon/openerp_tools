...
PARSER.add_option("-b", "--passwordb", dest="passdb", default='SezorganP', help="Password User Postgres db")
...
OPTIONS.portc = '8069'
OPTIONS.hostc = '127.0.0.1'
OPTIONS.dbc = 'dec'
OPTIONS.protocolec = "http"

OPTIONS.hosts = '192.168.12.17'
OPTIONS.dbs = 'dec'
OPTIONS.ports = '10014'
OPTIONS.protocoles = "https"


if OPTIONS.users == 'terp':
    OPTIONS.users = OPTIONS.dbs[:1].upper() + 'admin' + OPTIONS.dbs[-1].upper()
    OPTIONS.passwds = OPTIONS.dbs[-1].upper() + 'ezorgan' + OPTIONS.dbs[:1].upper()

if OPTIONS.userc == 'terp':
    OPTIONS.userc = OPTIONS.dbc[:1].upper() + 'admin' + OPTIONS.dbc[-1].upper()
    OPTIONS.passwdc = OPTIONS.dbc[-1].upper() + 'ezorgan' + OPTIONS.dbc[:1].upper()
...
for invoice_id in invoice_ids:
    ...
    if invoice_id:
        ...
        for invoice_line_id in invoice_line_ids:
            ...
            print CONNECTION_CIBLE.create('account.invoice.line', invoice_line)        
