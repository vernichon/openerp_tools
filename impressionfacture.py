...
invoice_ids = obj.execute(base, uid, pwd, 'account.invoice', 'search', [('type', '=', 'out_invoice')], 0, 8000)
for invoice_id in invoice_ids:
    datas = {'report_type': 'pdf', 'model': model, 'id': invoice_id}
    ctx = {'lang': 'fr_FR', 'tz': 'Europe/Paris'}

    invoice = obj.execute(base, uid, pwd, 'account.invoice', 'read', invoice_id, ['partner_id'])
    #print invoice['partner_id']
    report_id = sock.report(base, uid, pwd, name, [invoice_id], datas, ctx)
    ...
    while not state:
        try:
            ...
            print
            invoice['partner_id'][1].encode('iso-8859-15')
